...
//------------------------------------------------------------------------------
/*! \brief ContainSim6 custom constructor.
  
    ContainSim6 contains all the information to make a complete simulation run,
    ...
    m_maxFireTime(maxFireTime)
{
    ...
    double attackTime = m_force->firstArrival( LeftFlank6 );

    // Create the left flank
	  m_left = new Contain6( reportSize, reportRate, 
        diurnalROS,fireStartMinutesStartTime,
        lwRatio, distStep,
        LeftFlank6, force, attackTime, tactic, attackDist );
...
//------------------------------------------------------------------------------
/*! \brief ContainSim6 destructor.
 */

Sem::ContainSim6::~ContainSim6( void )
{
...
double Sem::ContainSim6::attackDistance( void ) const
{
...
double Sem::ContainSim6::attackPointX( void ) const
{
...
double Sem::ContainSim6::attackPointY( void ) const
{
...
double Sem::ContainSim6::attackTime( void ) const
{
...
void Sem::ContainSim6::checkmem( const char* fileName, int lineNumber, void* ptr,
...
double Sem::ContainSim6::distanceStep( void ) const
{
...
double Sem::ContainSim6::finalFireCost( void ) const
{
    return( m_finalCost );
}
...
double Sem::ContainSim6::finalFireLine( void ) const
{
    return( m_finalLine );
}
...
double Sem::ContainSim6::finalFirePerimeter( void ) const
{
    return( m_finalPerim );
}
...
double Sem::ContainSim6::finalFireSize( void ) const
{
    return( m_finalSize );
}
...
double Sem::ContainSim6::finalFireSweep( void ) const
{
    return( m_finalSweep );
}
...
double Sem::ContainSim6::finalFireTime( void ) const
{
    return( m_finalTime );
}
...
int Sem::ContainSim6::finalResourcesUsed( void ) const
{
    return( m_used );
}
...
void Sem::ContainSim6::finalStats( void )
{
    // Final time
    ...
    m_finalSize = 0.;
    if ( m_left->m_status == Sem::Contain6::Contained6 )
    {
...
double Sem::ContainSim6::fireBackAtAttack( void ) const
{
...
double Sem::ContainSim6::fireBackAtReport( void ) const
{
...
double Sem::ContainSim6::fireHeadAtAttack( void ) const
{
...
double Sem::ContainSim6::fireHeadAtReport( void ) const
{
...
double* Sem::ContainSim6::fireHeadX( void ) const
{
    return( m_h );
}
...
double Sem::ContainSim6::fireLwRatioAtReport( void ) const
{
...
double* Sem::ContainSim6::firePerimeterX( void ) const
{
    return( m_x );
}
...
double* Sem::ContainSim6::firePerimeterY( void ) const
{
    return( m_y );
}
...
int Sem::ContainSim6::firePoints( void ) const
{
    return( m_size );
}
...
double Sem::ContainSim6::fireReportTime( void ) const
{
...
double Sem::ContainSim6::fireSpreadRateAtBack( void ) const
{
...
double Sem::ContainSim6::fireSizeAtReport( void ) const
{
...
double Sem::ContainSim6::fireSpreadRateAtReport( void ) const
{
...
Sem::ContainForce6* Sem::ContainSim6::force( void ) const
{
    return( m_left->force() );
}
...
int Sem::ContainSim6::maximumSimulationSteps( void ) const
{
    return( m_maxSteps );
}
...
int Sem::ContainSim6::minimumSimulationSteps( void ) const
{
    return( m_minSteps );
}
...
//    double maxArea = 500.0;
    ...
    while ( rerun )
    {
        ...
        totalArea=0.0;
        suma = sumb = sumDT = 0.0;
        while ( m_left->m_status != Sem::Contain6::Overrun6
             && m_left->m_status != Sem::Contain6::Contained6
...
//                iLeft, m_u[iLeft], m_h[iLeft], elapsed );
        ...
        if ( m_left->m_status == Sem::Contain6::Contained6
          && m_left->m_tactic == Sem::Contain6::HeadAttack6 )
        {
        ...
        if ( m_left->m_status == Sem::Contain6::Overrun6 )
        {
            ...
            // Case 2: Try initial attack after more forces have arrived
                ...
                m_left->m_exhausted, LeftFlank6 ) ) > 0.01 )
            {
                m_left->containLog( ( logLevel >= 1 ),
                    ...
                    at, m_force->productionRate( at, LeftFlank6 ) );
                m_pass++;
            ...
            else
            {
                ...
                m_left->containLog( ( logLevel >= 1 ),
                    ...
                    "    - FIRE ESCAPES at %3.1f minutes\n",
                    m_pass, elapsed, m_left->m_step, elapsed );
                m_left->m_status = Sem::Contain6::Exhausted6;
            }
        }
        ...
        else if ( m_left->m_status == Sem::Contain6::Contained6 )
        {
...
//                	cout << m_pass << " "  << totalArea << " " <<m_maxFireSize << " " << "\n";
                	...
                	m_left->m_status = Sem::Contain6::SizeLimitExceeded6;
 //               } 
		...
		//------------------------------------------------------------------
			 ...
			 {     m_left->m_currentTime=m_maxFireTime;
    	           m_left->m_status = Sem::Contain6::TimeLimitExceeded6;
    ...
    //	m_left->m_status = Sem::Contain6::TimeLimitExceeded;
    //}
...
Sem::Contain6::ContainStatus6 Sem::ContainSim6::status( void ) const
{
...
Sem::Contain6::ContainTactic6 Sem::ContainSim6::tactic( void ) const
{
    return( m_left->tactic() );
}

// Calculate the size of the uncontained portion of the fire ellipse  by DT 1/2013
double Sem::ContainSim6::UncontainedArea( double head, double lwRatio, double x, double y,
										 Sem::Contain6::ContainTactic6 tactic )
{
        if( lwRatio < 1.0 )
            lwRatio = 1.00000001;
	...
	if ( tactic == Sem::Contain6::HeadAttack6 )
